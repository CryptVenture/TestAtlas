...
lifecycle:
  ...
  - history/run_log.md
boundary: Does NOT execute product behavior. Does NOT mutate any artifact other than the lifecycle/brain update files. Read-only over `_testatlas/brain/` plus a single state.last_command bump on close.
---

# TestAtlas Command (V2 core): status
