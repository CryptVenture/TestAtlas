...
command: retest
version: 1.0.0
description: Re-execute the original repro for issues with status=fixed_pending_retest; transition to closed (recovered) or reopened (still failing); append append-only retest history; capture new evidence.
...
lifecycle:
  ...
  - history/run_log.md
boundary: Does NOT skip the actual re-execution. Does NOT mutate prior history entries (append-only). Does NOT fabricate retest results — must capture new evidence first. Does NOT close issues with status≠fixed_pending_retest. Does NOT mutate target-repo source files.
---

# TestAtlas Command: retest
