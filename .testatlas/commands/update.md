...
lifecycle:
  ...
  - history/run_log.md
boundary: NEVER auto-applies updates without explicit operator confirmation. NEVER touches `_testatlas/` workspace data — updates are scoped to the `.testatlas/` suite tree. NEVER bypasses the workspace lockfile (UPDATE-06). Refuses to update when in-flight test runs are detected. Honors `disableUpdateCheck` and pinning semantics (UPDATE-03, UPDATE-04).
---

# TestAtlas Command: update
