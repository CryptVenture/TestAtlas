...
lifecycle:
  ...
  - history/run_log.md
boundary: Does NOT mutate the receiving sub-agent's workspace. Does NOT auto-invoke the sub-agent. Records context boundaries only — execution remains the sub-agent's responsibility. Does NOT cross the two-tree invariant (writes only under `_testatlas/handoffs/`).
---

# TestAtlas Command: handoff
