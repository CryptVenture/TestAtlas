...
lifecycle:
  ...
  - history/run_log.md
boundary: Does NOT delete issues (closed-as-consolidated is a status, not deletion). Does NOT drop evidence on merge. Does NOT mutate prior scorecard history (append-only). Does NOT fabricate severity inheritance — derives from group members verbatim. Does NOT mutate target-repo source files.
---

# TestAtlas Command: consolidate
...
4. **Refresh `_testatlas/13_quality_scorecard.md`** with these four longitudinal series, each preserved across runs via generated-section markers (Phase 2 contract — `<!-- testatlas:generated:start --> ... <!-- testatlas:generated:end -->` — human content outside the markers is left intact):
