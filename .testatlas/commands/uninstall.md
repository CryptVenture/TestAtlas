...
lifecycle:
  ...
  - history/run_log.md
boundary: NEVER auto-runs without operator invocation. NEVER touches target-repo source files. By default removes only `.testatlas/`; `--purge` opt-in is required to remove `_testatlas/`. Refuses without `--force-untracked` if the install manifest is missing or invalid (avoids blind nuking of non-suite-managed files).
---

# TestAtlas Command: uninstall
