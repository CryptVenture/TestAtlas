...
lifecycle:
  ...
  - history/run_log.md
boundary: Does NOT trigger destructive jobs (e.g. nightly user-purge, payment captures). Does NOT mutate job source. Does NOT fabricate queue depths or retry counts when shell unavailable — degrade per bootstrap §4.
---

# TestAtlas Command (V2): explore-jobs
...
4. Inspect `_testatlas/12_app_map.json.jobs` and `_testatlas/maps/jobs.json`.
