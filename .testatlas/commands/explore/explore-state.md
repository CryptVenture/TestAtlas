...
lifecycle:
  ...
  - history/run_log.md
boundary: Does NOT navigate to production hosts when allowProductionTesting=false. Does NOT trigger destructive UI flows. Does NOT fabricate state captures when MCP/browser unavailable — degrade per bootstrap §4.
---

# TestAtlas Command (V2): explore-state
...
4. Inspect `_testatlas/12_app_map.json` and `_testatlas/maps/states.json`.
