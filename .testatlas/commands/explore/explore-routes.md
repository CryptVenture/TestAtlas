...
lifecycle:
  ...
  - history/run_log.md
boundary: Does NOT mutate route source. Does NOT navigate to production hosts when allowProductionTesting=false. Does NOT fabricate route lists when MCP/browser unavailable — degrade per bootstrap §4.
---

# TestAtlas Command (V2): explore-routes
...
4. Inspect `_testatlas/12_app_map.json` and `_testatlas/maps/routes.json`.
