...
lifecycle:
  ...
  - history/run_log.md
boundary: Spawns recommended sub-explorers via the host's subagent-spawn capability when available; aggregates findings into _testatlas/02_product_overview.md alongside the routing record at _testatlas/explore-plan.md. Does NOT write evidence — children own their evidence directories.
---

# TestAtlas Command (V2): explore-all
...
4. Inspect `_testatlas/12_app_map.json` and prior `_testatlas/explore-plan.md`.
