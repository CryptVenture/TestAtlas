---
command: explore-release-readiness
version: 2.0.0
...
lifecycle:
  ...
  - history/run_log.md
boundary: Read-only over `_testatlas/`. Does NOT mutate release artifacts (CHANGELOG, version files, tags). Does NOT trigger releases. Does NOT fabricate readiness signals when shell unavailable — degrade per bootstrap §4.
---

# TestAtlas Command (V2): explore-release-readiness
...
4. Inspect `_testatlas/brain/state.json`, `_testatlas/brain/issues.json`, `_testatlas/brain/coverage.json`.
