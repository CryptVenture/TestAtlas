...
lifecycle:
  ...
  - history/run_log.md
boundary: NEVER attempts exploitation, fuzzing, credential stuffing, or active probing. NEVER persists secret values (redact before write). NEVER exfiltrates data outside `_testatlas/`. Does NOT fabricate auth flows, secret-handling locations, or redaction gaps when shell unavailable — degrade per bootstrap §4.
---

# TestAtlas Command: explore-security
