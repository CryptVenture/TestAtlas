...
lifecycle:
  ...
  - history/run_log.md
boundary: Does NOT mutate domain/flow/issue/evidence source files. Only writes `_testatlas/brain/quality_scores.json` (atomic) and updates lifecycle / generated-marker sections in the quality_scores report.
---

# TestAtlas Command (V2 brain): brain-score
...
4. Inspect `_testatlas/brain/coverage.json`, `evidence.json`, `issues.json`, `domains.json`, `flows.json`, `agent_sessions.json`, `decisions.json`, `drift.json`.
