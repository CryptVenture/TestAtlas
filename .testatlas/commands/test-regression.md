...
lifecycle:
  ...
  - history/run_log.md
boundary: Does NOT run destructive scenarios when allowDestructiveActions=false. Does NOT run against production when allowProductionTesting=false. Does NOT fabricate diff results — must capture new evidence first. Does NOT mutate the prior baseline RUN; the diff is one-directional.
---

# TestAtlas Command: test-regression
