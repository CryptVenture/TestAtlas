<!-- TestAtlas command: atlas-council-product-review. Paste .testatlas/bootstrap.md first; description: Debate-mode council on product priority, feature coherence, and tradeoffs — personas argue for/against a conclusion through the 9-round protocol. -->
...
First read `.testatlas/bootstrap.md`. Then read `prompts/atlas-council-product-review.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
