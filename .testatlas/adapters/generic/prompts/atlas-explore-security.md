<!-- TestAtlas command: atlas-explore-security. Paste .testatlas/bootstrap.md first; description: Catalog auth surfaces, secrets-handling locations, and redaction risks per PRD §6.5 — read-only defensive audit; never attempts exploitation; never persists secret values. -->
...
First read `.testatlas/bootstrap.md`. Then read `prompts/atlas-explore-security.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
