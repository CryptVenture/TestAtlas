<!-- TestAtlas command: atlas-test-generate-scenarios. Paste .testatlas/bootstrap.md first; description: Generate exploratory charters and manual test scenarios from documented flows under `_testatlas/flows/`. Output marked `generated-not-yet-validated` until executed. -->
...
First read `.testatlas/bootstrap.md`. Then read `prompts/atlas-test-generate-scenarios.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
