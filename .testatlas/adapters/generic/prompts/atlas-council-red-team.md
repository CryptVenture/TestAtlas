<!-- TestAtlas command: atlas-council-red-team. Paste .testatlas/bootstrap.md first; description: Red Team Challenge — adversarial personas attempt to find hidden risks and invalidate confident claims through the 9-round protocol. -->
...
First read `.testatlas/bootstrap.md`. Then read `prompts/atlas-council-red-team.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
