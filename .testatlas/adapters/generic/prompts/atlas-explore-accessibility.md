<!-- TestAtlas command: atlas-explore-accessibility. Paste .testatlas/bootstrap.md first; description: Evaluate keyboard nav, focus, labels, semantics, contrast, dynamic feedback per PRD §13.9 via mandatory Chrome DevTools MCP a11y walkthrough (lighthouse_audit + ARIA introspection); degrade to code-reading without MCP. -->

<!-- TESTATLAS:GENERATED:START section="adapter-body" source="commands/explore-accessibility.md" hash="3d41f63931a54b73b90c57205cb138312b0fdb94dced848143306f528542e561" -->
First read `.testatlas/bootstrap.md`. Then read `prompts/atlas-explore-accessibility.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
...
5. Connect to Chrome DevTools MCP and confirm the canonical accessibility toolset is reachable. The required tools (verbatim names) are:
   ...
   - `wait_for({ text: string[], timeout?: number })` — text-based readiness wait; this is the only valid form (no selector, network-idle, or settle parameter). For selector/readiness signals, follow with `evaluate_script` polling per `.testatlas/reference/chrome-devtools-mcp.md`.
   ...
   - `lighthouse_audit({ mode: "navigation" })` — accepts `{ device?, mode?, outputDirPath? }` only (no `categories`/`category` param). The tool returns Accessibility, SEO, Best-Practices, and Agentic-browsing categories on every call; the agent extracts the `accessibility` slice manually from the JSON result and persists per-issue findings plus the category score.
...
6. For each route in `_testatlas/12_app_map.json`, navigate, wait for the route to settle (text-based `wait_for({text: [...], timeout?})`), then invoke `lighthouse_audit({ mode: "navigation" })`. The tool has no `categories`/`category` filter — it returns Accessibility, SEO, Best-Practices, and Agentic-browsing on every call; extract the `accessibility` slice from the returned JSON manually. Persist the full lighthouse JSON under `_testatlas/evidence/explore-accessibility/<timestamp>/<route-slug>/lighthouse.json`. Record the accessibility category score and the per-issue list.
