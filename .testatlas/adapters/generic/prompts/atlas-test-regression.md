<!-- TestAtlas command: atlas-test-regression. Paste .testatlas/bootstrap.md first; description: Re-run previously-failed scenarios from prior RUN-<timestamp>.json files; diff against the prior failed run; report regressed / recovered / unchanged / unverified per scenario. -->
...
First read `.testatlas/bootstrap.md`. Then read `prompts/atlas-test-regression.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
