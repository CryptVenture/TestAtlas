<!-- TestAtlas command: atlas-generate-retest-pack. Paste .testatlas/bootstrap.md first; description: Generate self-contained retest packs from open issue records under `_testatlas/to_fix/`. Each pack carries reproduction steps, pass/fail criteria, evidence refs, and fixtures so any agent can re-verify a fix. -->
