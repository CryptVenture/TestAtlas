<!-- TestAtlas command: atlas-explore-observability. Paste .testatlas/bootstrap.md first; description: Map logging setup, metrics, alerts, and tracing — verify log generation, metric collection, alert triggers, and distributed-trace propagation. Static audit + live probe when shell available. -->
...
First read `.testatlas/bootstrap.md`. Then read `prompts/atlas-explore-observability.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
