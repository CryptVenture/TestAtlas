<!-- TestAtlas command: atlas-explore-routes. Paste .testatlas/bootstrap.md first; description: Map every route, navigation paths, guards, redirects, deep-link behavior, history (back/forward), and per-route ownership via mandatory Chrome DevTools MCP walkthrough; degrade to code-reading when MCP unavailable. -->
...
First read `.testatlas/bootstrap.md`. Then read `prompts/atlas-explore-routes.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
