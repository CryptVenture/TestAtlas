<!-- TestAtlas command: atlas-test-generate-automation. Paste .testatlas/bootstrap.md first; description: Generate framework-specific automation skeletons (Playwright, Cypress, API, CLI, contract, smoke) from documented flows. Output marked `generated-but-not-validated` until executed. -->
...
First read `.testatlas/bootstrap.md`. Then read `prompts/atlas-test-generate-automation.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
