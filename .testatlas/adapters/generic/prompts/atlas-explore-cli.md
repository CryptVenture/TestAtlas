<!-- TestAtlas command: atlas-explore-cli. Paste .testatlas/bootstrap.md first; description: Map package scripts, binaries, and task runners for the target product; classify destructive vs safe commands; capture help text and exit codes for safe ones. -->
...
First read `.testatlas/bootstrap.md`. Then read `prompts/atlas-explore-cli.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
