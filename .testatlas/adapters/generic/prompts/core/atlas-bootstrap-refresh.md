<!-- TestAtlas command: atlas-bootstrap-refresh. Paste .testatlas/bootstrap.md first; description: Re-read the constitution, validate token budget, and refresh bootstrap shards so a long-running agent doesn't drift from the rules. -->
