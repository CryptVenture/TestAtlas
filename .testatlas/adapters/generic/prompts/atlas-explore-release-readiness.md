<!-- TestAtlas command: atlas-explore-release-readiness. Paste .testatlas/bootstrap.md first; description: Map release artifacts, blockers, readiness state, version tags, and gates. Synthesizes signal from prior explorers into a release/no-go decision report. -->

<!-- TESTATLAS:GENERATED:START section="adapter-body" source="commands/explore/explore-release-readiness.md" hash="6253a04f8ce361bead898233363905751854d3454f1eb81a1afc3d86714bf428" -->
First read `.testatlas/bootstrap.md`. Then read `prompts/atlas-explore-release-readiness.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
...
Then run `node .testatlas/scripts/update-brain-after-command.js --command explore-release-readiness --actor agent --summary "Assessed release readiness across V2 quality signals" --status completed --reindex`.
