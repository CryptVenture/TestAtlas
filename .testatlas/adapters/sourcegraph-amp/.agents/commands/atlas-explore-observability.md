<!-- TestAtlas command: atlas-explore-observability. Invoke as /atlas-explore-observability. Description: Map logging setup, metrics, alerts, and tracing — verify log generation, metric collection, alert triggers, and distributed-trace propagation. Static audit + live probe when shell available. -->
...
First read `.testatlas/bootstrap.md`. Then read `.agents/commands/atlas-explore-observability.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
