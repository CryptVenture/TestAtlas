<!-- TestAtlas command: atlas-explore-api. Invoke as /atlas-explore-api. Description: Map REST/GraphQL/RPC/server-action/webhook/event-consumer surfaces; capture contracts, auth, errors, pagination; safely probe sandbox endpoints. -->
...
First read `.testatlas/bootstrap.md`. Then read `.agents/commands/atlas-explore-api.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
