<!-- TestAtlas command: atlas-explore-integrations. Invoke as /atlas-explore-integrations. Description: Map auth, payments, email, analytics, storage, webhooks, and feature-flag integrations; distinguish sandbox/test/prod endpoints; probe sandbox only when safe. -->
...
First read `.testatlas/bootstrap.md`. Then read `.agents/commands/atlas-explore-integrations.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
