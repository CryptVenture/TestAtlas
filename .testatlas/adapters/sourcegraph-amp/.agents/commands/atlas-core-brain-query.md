<!-- TestAtlas command: atlas-core-brain-query. Invoke as /atlas-core-brain-query. Description: Answer a question about the workspace by reading brain JSON; cite file paths for every claim. -->
...
First read `.testatlas/bootstrap.md`. Then read `.agents/commands/atlas-core-brain-query.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
