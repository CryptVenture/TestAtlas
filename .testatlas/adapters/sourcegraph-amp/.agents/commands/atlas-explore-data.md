<!-- TestAtlas command: atlas-explore-data. Invoke as /atlas-explore-data. Description: Map schemas, entities, lifecycle states, seed fixtures, queues, caches, and storage objects from local schema introspection; never read or persist production rows. -->

<!-- TESTATLAS:GENERATED:START section="adapter-body" source="commands/explore-data.md" hash="6e90f0ed43eb2fdc0f11b5acd1bc5a05a5f7403fb412fcd8bf16f1fea2bcedf0" -->
First read `.testatlas/bootstrap.md`. Then read `.agents/commands/atlas-explore-data.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
...
- **objective:** Map the data layer — schemas, models, migrations, seed data shapes, queue topics, cache prefixes, object-storage buckets — of the target product.
- **scope:** Every ORM model, schema definition, migration file, seed/fixture file, and queue/cache/storage reference in the codebase. Excludes runtime data introspection — this command reads code, not data stores.
- **files-to-read:** `_testatlas/12_app_map.json` (REQUIRED); `.testatlas/schemas/app-map.schema.json`; `.testatlas/default.config.json` (`safeMode`, `allowProductionTesting`); ORM/schema files (`prisma/schema.prisma`, `db/schema.rb`, `models/`, `migrations/`, `alembic/`, `drizzle/schema.*`, `entities/`, Mongoose schemas, GORM struct tags, `*.dbml`); seed and fixture files; queue/cache/storage constants in source.
- **output-format:** `data` entries in `12_app_map.json` validating against `app-map.schema.json` (one per model/table/topic/bucket); evidence (schema dumps, migration listings, fixture-shape captures) under `_testatlas/evidence/explore-data/<timestamp>/`.
...
- **exit-criteria:** Every model/migration/seed/topic/bucket cataloged with file path; no production data store contacted; schema validation passes.
...
- `_testatlas/12_app_map.json` `entities[]` (and related ID arrays) — closed string arrays of data-surface IDs; per `app-map.schema.json` this field is `array<string>` and the document declares `additionalProperties:false`, so rich data-surface payloads do NOT live in this file.
