<!-- TestAtlas command: atlas-test-domain. Invoke as /atlas-test-domain. Description: Execute domain-scoped scenarios across PRD §26 modes (negative/state/integration/setup-testability); state-typed UI scenarios drive the mandatory Chrome DevTools MCP state-coverage walkthrough (5 states); scenario.type selects mode. -->

<!-- TESTATLAS:GENERATED:START section="adapter-body" source="commands/test-domain.md" hash="18f089e470b5d58f94eed6a79e4532f701ec1525ec2527b2f53cddc1d92f463a" -->
First read `.testatlas/bootstrap.md`. Then read `.agents/commands/atlas-test-domain.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
