<!-- TestAtlas command: atlas-bootstrap-refresh. Invoke as /atlas-bootstrap-refresh. Description: Re-read the constitution, validate token budget, and refresh bootstrap shards so a long-running agent doesn't drift from the rules. -->
