<!-- TestAtlas command: atlas-explore-security-privacy. Invoke as /atlas-explore-security-privacy. Description: Map auth flows, permission boundaries, sensitive-data handling, injection risks, and privacy controls. V2 expansion of V1 explore-security with mandatory walkthrough when browser+MCP available. -->
...
First read `.testatlas/bootstrap.md`. Then read `.agents/commands/atlas-explore-security-privacy.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
