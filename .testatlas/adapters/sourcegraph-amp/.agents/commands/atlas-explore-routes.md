<!-- TestAtlas command: atlas-explore-routes. Invoke as /atlas-explore-routes. Description: Map every route, navigation paths, guards, redirects, deep-link behavior, history (back/forward), and per-route ownership via mandatory Chrome DevTools MCP walkthrough; degrade to code-reading when MCP unavailable. -->
...
First read `.testatlas/bootstrap.md`. Then read `.agents/commands/atlas-explore-routes.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
