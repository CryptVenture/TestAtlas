<!-- TestAtlas command: atlas-init. Invoke as /atlas-init. Description: Bootstrap the _testatlas/ workspace tree in a target repository — 23 subdirs, 14 canonical files, and a project manifest — idempotently. -->
...
First read `.testatlas/bootstrap.md`. Then read `.agents/commands/atlas-init.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
