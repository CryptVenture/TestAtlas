<!-- TestAtlas command: atlas-brain-drift. Invoke as /atlas-brain-drift. Description: Detect drift between the last exploration and the current repository state and write _testatlas/brain/drift.json with per-domain/flow drift status. -->
...
First read `.testatlas/bootstrap.md`. Then read `.agents/commands/atlas-brain-drift.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
