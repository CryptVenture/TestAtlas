<!-- TestAtlas command: atlas-generate-retest-pack. Invoke as /atlas-generate-retest-pack. Description: Generate self-contained retest packs from open issue records under `_testatlas/to_fix/`. Each pack carries reproduction steps, pass/fail criteria, evidence refs, and fixtures so any agent can re-verify a fix. -->
