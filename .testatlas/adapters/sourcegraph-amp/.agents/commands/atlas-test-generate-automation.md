<!-- TestAtlas command: atlas-test-generate-automation. Invoke as /atlas-test-generate-automation. Description: Generate framework-specific automation skeletons (Playwright, Cypress, API, CLI, contract, smoke) from documented flows. Output marked `generated-but-not-validated` until executed. -->
...
First read `.testatlas/bootstrap.md`. Then read `.agents/commands/atlas-test-generate-automation.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
