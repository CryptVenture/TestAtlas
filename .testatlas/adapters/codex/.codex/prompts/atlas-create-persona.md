<!-- TestAtlas command: atlas-create-persona. Invoke as /prompts:atlas-create-persona. Description: Author a new persona (system, generated, or project scope) by invoking scripts/create-persona.js — emits persona.{md,json} pair under _testatlas/agents/personas/<type>/<id>.{md,json} and updates brain/personas.json. -->
...
First read `.testatlas/bootstrap.md`. Then read `.codex/prompts/atlas-create-persona.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
