<!-- TestAtlas command: atlas-council-bug-triage. Invoke as /prompts:atlas-council-bug-triage. Description: Bug triage council — multiple personas classify and prioritize open issues by severity, priority, and remediation sequencing through the 9-round protocol. -->
...
First read `.testatlas/bootstrap.md`. Then read `.codex/prompts/atlas-council-bug-triage.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
