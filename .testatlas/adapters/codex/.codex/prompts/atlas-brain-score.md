<!-- TestAtlas command: atlas-brain-score. Invoke as /prompts:atlas-brain-score. Description: Compute the 11 PRD §7.15 quality scores from documented brain evidence and write _testatlas/brain/quality_scores.json with freshness + confidence + disclaimer. -->
...
First read `.testatlas/bootstrap.md`. Then read `.codex/prompts/atlas-brain-score.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
