<!-- TestAtlas command: atlas-report-domain. Invoke as /prompts:atlas-report-domain. Description: Render a domain-scoped report combining quality scores, issues, coverage, drift, and recommendations into _testatlas/reports/domain-<slug>.md. -->
...
First read `.testatlas/bootstrap.md`. Then read `.codex/prompts/atlas-report-domain.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
