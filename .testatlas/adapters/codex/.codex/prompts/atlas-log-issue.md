<!-- TestAtlas command: atlas-log-issue. Invoke as /prompts:atlas-log-issue. Description: Capture a quality finding as an issue under to_fix/ with severity, confidence, evidence references, and back-links to flows/domains per PRD §17. -->
...
First read `.testatlas/bootstrap.md`. Then read `.codex/prompts/atlas-log-issue.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
