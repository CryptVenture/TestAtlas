<!-- TestAtlas command: atlas-core-bootstrap-refresh. Invoke as /prompts:atlas-core-bootstrap-refresh. Description: Re-read the constitution, validate token budget, and refresh bootstrap shards so a long-running agent doesn't drift from the rules. -->
...
First read `.testatlas/bootstrap.md`. Then read `.codex/prompts/atlas-core-bootstrap-refresh.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
