<!-- TestAtlas command: atlas-maintain-migrate. Invoke as /prompts:atlas-maintain-migrate. Description: Migrate a V1 TestAtlas workspace to V2 via `scripts/v2-migrate.js`. Backs up the existing workspace, creates V2 brain + agent + maps + stories + tests directories, populates baseline brain JSON, and bumps the manifest. Idempotent and rollback-safe. -->
