<!-- TestAtlas command: atlas-explore-cli. Invoke as /prompts:atlas-explore-cli. Description: Map package scripts, binaries, and task runners for the target product; classify destructive vs safe commands; capture help text and exit codes for safe ones. -->
...
First read `.testatlas/bootstrap.md`. Then read `.codex/prompts/atlas-explore-cli.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
...
- `_testatlas/12_app_map.json` `cliCommands[]` — closed string array of CLI command IDs (e.g. `CLI-deploy-staging`); per `app-map.schema.json` this field is `array<string>` and the document declares `additionalProperties:false`, so rich command payloads do NOT live in this file.
