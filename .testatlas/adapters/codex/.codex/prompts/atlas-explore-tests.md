<!-- TestAtlas command: atlas-explore-tests. Invoke as /prompts:atlas-explore-tests. Description: Inventory existing tests, measure coverage, identify gaps, surface flaky tests. Static audit + live test-runner probe when shell available. -->
...
First read `.testatlas/bootstrap.md`. Then read `.codex/prompts/atlas-explore-tests.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
