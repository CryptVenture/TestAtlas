...
This adapter ships TestAtlas's 30 commands as a single concatenated `.rules` file at the repository root. Zed reads project rules from `.rules` (no extension; the file is markdown despite the missing `.md` extension), so every TestAtlas command is visible to the agent at all times.
