<!-- TestAtlas command: atlas-test-all. Invoke as /atlas-test-all.md. Description: Umbrella test orchestrator — runs `/atlas:test-flow --all` AND `/atlas:test-domain --all` and aggregates per-child run records into a single merged RUN-<timestamp>.{md,json}. -->
...
First read `.testatlas/bootstrap.md`. Then read `.clinerules/workflows/atlas-test-all.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
