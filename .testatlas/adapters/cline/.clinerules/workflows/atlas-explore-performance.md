<!-- TestAtlas command: atlas-explore-performance. Invoke as /atlas-explore-performance.md. Description: Detect user-visible slowness, blocking interactions, retries, reliability per PRD §13.10 via mandatory Chrome DevTools MCP perf walkthrough (baseline + throttled traces, performance_analyze_insight); degrade to code-reading without MCP. -->
...
First read `.testatlas/bootstrap.md`. Then read `.clinerules/workflows/atlas-explore-performance.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
