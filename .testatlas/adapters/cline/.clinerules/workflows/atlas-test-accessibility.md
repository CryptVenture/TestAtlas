<!-- TestAtlas command: atlas-test-accessibility. Invoke as /atlas-test-accessibility.md. Description: Execute accessibility-typed scenarios via mandatory Chrome DevTools MCP a11y walkthrough (lighthouse_audit + ARIA introspection); assert against PRD §13.9 thresholds; emit RUN-<timestamp>.{md,json} with per-scenario a11y findings. -->

<!-- TESTATLAS:GENERATED:START section="adapter-body" source="commands/test-accessibility.md" hash="23f1761b2431baddd51a516c9e00d122105bdc2552edcadfd4d182b1874043e7" -->
First read `.testatlas/bootstrap.md`. Then read `.clinerules/workflows/atlas-test-accessibility.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
