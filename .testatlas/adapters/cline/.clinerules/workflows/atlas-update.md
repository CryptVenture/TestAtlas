<!-- TestAtlas command: atlas-update. Invoke as /atlas-update.md. Description: Invoke the suite self-update flow — checks GitHub Releases per UPDATE-01, delegates to Phase 7 update.js for atomic apply with backup, never auto-applies without operator confirmation. -->
...
First read `.testatlas/bootstrap.md`. Then read `.clinerules/workflows/atlas-update.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
