<!-- TestAtlas command: atlas-report-dashboard-data. Invoke as /atlas-report-dashboard-data.md. Description: Render a machine-readable dashboard data export (PRD §16) at _testatlas/reports/dashboard-data.json suitable for downstream UIs and CI status pages. -->
...
First read `.testatlas/bootstrap.md`. Then read `.clinerules/workflows/atlas-report-dashboard-data.md` (already loaded into your context if invoked via slash). Follow both exactly. If they conflict, bootstrap safety and persistence rules win unless this command is more specific and not less safe.
