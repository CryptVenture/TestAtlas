...
This adapter ships TestAtlas's 30 commands as a single concatenated rules file at `.amazonq/rules/atlas.md`. Amazon Q auto-applies project rules from `.amazonq/rules/`; the user prompt library at `~/.aws/amazonq/prompts/` provides machine-wide alternative for the same content.
...
Amazon Q auto-applies every file in `.amazonq/rules/` into the system context. Shipping 30 separate command files would 30× the prompt-cache invalidation surface — any single edit to any source command would invalidate the cache for all chats. A single concatenated `atlas.md` keeps the cache stable.
