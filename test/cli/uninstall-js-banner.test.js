// test/cli/uninstall-js-banner.test.js
//
// Quick 260506-h9q — assert that `node scripts/uninstall.js --help` renders
// the TESTATLAS block-art banner.
...
const UNINSTALL_JS = path.join(SUITE_ROOT, 'scripts', 'uninstall.js');
const ANSI_ESC = String.fromCharCode(0x1b);

function runHelp(env = {}) {
  return spawnSync('node', [UNINSTALL_JS, '--help'], {
    cwd: SUITE_ROOT,
    encoding: 'utf8',
    timeout: 10_000,
    env: { ...process.env, ...env },
...
test('uninstall.js --help: NO_UNICODE=1 → ASCII `#` fallback (not `█`)', () => {
