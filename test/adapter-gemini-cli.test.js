...
//
// Structural assertions on the 30 generated Gemini CLI adapter commands.
...
test('Test 1: 30 derived atlas-*.toml files exist (one per source command)', async () => {
  ...
  assert.equal(derived.length, 30, `expected 30 derived TOML files; got ${derived.length}`);
