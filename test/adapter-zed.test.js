...
// `.testatlas/adapters/zed/.rules` file. Same shape as Aider's
// CONVENTIONS.md but at file path `.rules` (no extension).
...
import { renderZed } from '../scripts/lib/adapters/render-zed.js';
...
test('Test 1: .rules exists and total line count ≤ K × 7 + 16 envelope/header', async () => {
  ...
  assert.ok(lines.length <= cap, `.rules must be ≤${cap} lines; got ${lines.length}`);
});
...
test('Test 3: single envelope; aggregate hash = hashContent(join(per-source hashes))', async () => {
  ...
  assert.ok(marker, '.rules must contain a single adapter envelope');
...
`;
  ...
  assert.ok(threw, 'renderZed must throw when a section exceeds 7 lines');
  assert.match(threw.message, /super-long|max 7|7 lines/);
});

test('Test 5: README.md exists with required sections', async () => {
  const readmePath = path.join(repoRoot, '.testatlas', 'adapters', 'zed', 'README.md');
  ...
  assert.match(text, /\.rules/, 'README must reference .rules path');
});
