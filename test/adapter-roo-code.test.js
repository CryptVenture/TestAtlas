// test/adapter-roo-code.test.js
//
// Structural assertions on the SINGLE concatenated
// `.testatlas/adapters/roo-code/.roo/rules/atlas.md` file. Same shape as
// Aider's CONVENTIONS.md: 32 H2 atlas-* sections, ≤7 lines each, single
// adapter envelope wrapping the whole body, aggregate hash over all 32
// per-source hashes.
...
import { renderRooCode } from '../scripts/lib/adapters/render-roo-code.js';
...
`;
  ...
  assert.ok(threw, 'renderRooCode must throw when a section exceeds 7 lines');
  assert.match(threw.message, /super-long|max 7|7 lines/);
});

test('Test 5: README.md exists with required sections', async () => {
  const readmePath = path.join(repoRoot, '.testatlas', 'adapters', 'roo-code', 'README.md');
  ...
  assert.match(text, /one file/i, 'README must explain why one file, not 32');
