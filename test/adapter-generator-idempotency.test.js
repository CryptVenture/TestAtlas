 ...
 *   - .testatlas/commands/*.md (the 30 sources)
...
test('Test 6b: re-running into the same workspace writes nothing on second pass', async () => {
  ...
  try {
    ...
    assert.equal(first.adapters[0].written.length, 30, 'first run writes 30 files');
    assert.equal(second.adapters[0].written.length, 0, 'second run writes nothing');
    assert.equal(second.adapters[0].unchanged.length, 30, 'second run reports 30 unchanged');
  } finally {
