# 09 Artifact Index

> Updated automatically by every TestAtlas command. Human notes outside the generated blocks are preserved across runs.

This document is the index of every artifact in the workspace. Generated sections list the live artifact set; humans can add narrative notes outside the generated blocks (e.g., "we deprecated the old `payments-v1` domain on 2026-04-01").

## Canonical Documents

<!-- TESTATLAS:GENERATED:START section="canonical-docs" -->
- `00_overview.md`
- `01_system_map.md`
- `02_test_strategy.md`
- `03_execution_status.md`
- `04_open_questions.md`
- `05_assumptions.md`
- `06_risks_and_gaps.md`
- `07_environment_and_access.md`
- `08_glossary.md`
- `09_artifact_index.md`
- `10_command_log.md`
- `11_workspace_manifest.json`
- `12_app_map.json`
- `13_quality_scorecard.md`
<!-- TESTATLAS:GENERATED:END section="canonical-docs" -->

## Domain Documents

<!-- TESTATLAS:GENERATED:START section="domain-docs" -->
- domains/api/index.md
- domains/auth/index.md
- domains/dashboard/index.md
- domains/marketing/index.md
<!-- TESTATLAS:GENERATED:END section="domain-docs" -->

## Flow Documents

<!-- TESTATLAS:GENERATED:START section="flow-docs" -->
- flows/FLOW-api-health-check.md
- flows/FLOW-auth-login-with-credentials.md
- flows/FLOW-auth-logout.md
- flows/FLOW-auth-signup-new-account.md
- flows/FLOW-dashboard-dashboard-navigation.md
- flows/FLOW-marketing-view-landing-page.md
<!-- TESTATLAS:GENERATED:END section="flow-docs" -->

## Issue Documents

<!-- TESTATLAS:GENERATED:START section="issue-docs" -->
- to_fix/ISSUE-001-mock-jwt-token-literal-in-api-auth-login-route-handler.md
- to_fix/ISSUE-002-dashboard-layout-does-not-handle-expired-sessions.md
- to_fix/ISSUE-003-signup-form-lacks-password-strength-validation.md
<!-- TESTATLAS:GENERATED:END section="issue-docs" -->

## Evidence

<!-- TESTATLAS:GENERATED:START section="evidence" -->
- EVIDENCE-001
- EVIDENCE-002
- EVIDENCE-003
<!-- TESTATLAS:GENERATED:END section="evidence" -->

## Reports

<!-- TESTATLAS:GENERATED:START section="reports" -->
(no reports yet)
<!-- TESTATLAS:GENERATED:END section="reports" -->

## JSON Maps

<!-- TESTATLAS:GENERATED:START section="json-maps" -->
- `11_workspace_manifest.json`
- `12_app_map.json`
<!-- TESTATLAS:GENERATED:END section="json-maps" -->

## Command Outputs

<!-- TESTATLAS:GENERATED:START section="command-outputs" -->
(no commands invoked yet)
<!-- TESTATLAS:GENERATED:END section="command-outputs" -->

## Sub-Agent Outputs

<!-- TESTATLAS:GENERATED:START section="sub-agent-outputs" -->
(no sub-agent handoffs yet)
<!-- TESTATLAS:GENERATED:END section="sub-agent-outputs" -->
