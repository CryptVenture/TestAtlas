...
(What does the persona accomplish? Stated as a verb phrase: "checkout a cart", "reset a password", "invite a teammate".) POST /api/items with a missing title, expect 400 and an error object
...
(Which domain owns this flow? Link to `domains/<slug>/`.) domain-validation
