...
This is the first-hour, runnable walkthrough of the 32 `/atlas:*` commands. It assumes you have already installed the suite into your repo (e.g., `npx @webventures/testatlas init` — see [docs/INSTALL.md](INSTALL.md) if not). Everything below is what you run *inside your AI agent* once `.testatlas/` and `_testatlas/` are on disk. Read top-to-bottom and you'll go from a cold repo to a schema-validated, evidence-backed `_testatlas/REPORT-latest.md` without consulting any other doc first.
