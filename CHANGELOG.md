...
_No notable changes since 2.0.4._

## [2.0.4] - 2026-05-12

_No notable changes since 2.0.3._
...
- **`new URL(import.meta.url).pathname` replaced with `import.meta.dirname` (Node 20.11+).** On Windows, `new URL(import.meta.url).pathname` produces `/D:/a/TestAtlas/...` (leading slash); `path.resolve` then treats the leading-slash absolute path as a relative-to-current-drive path and prepends `D:\`, producing nonsense like `D:\D:\a\...`. The downstream `readFile` fails with ENOENT. Fixed in `test/install/adapter-detect-expanded.test.js` and `scripts/generate-dashboard-data.js` — both now use `import.meta.dirname` which is platform-correct.
- **Path separators in regex assertions normalized to forward slashes.** Two tests asserted that paths produced by their script-under-test matched POSIX-style regexes (`/agents\/personas\/system\/...$/`, `/scripts\/foo\.js:2$/`); on Windows the script returns `\` separators so the regex never matches. (1) `test/scripts/create-persona.test.js` now `.replaceAll('\\\\','/')`-normalizes `r.mdPath`/`r.jsonPath` before regex-match. (2) `scripts/explore-codebase.js` itself was emitting OS-native separators in its JSON output `source` field — fixed at the source so all downstream consumers (schema validators, agents, this test) see stable forward-slash paths regardless of host OS.
- **`bump-version-*.test.js` skipped on Windows (POSIX-only test harness).** The shared `test/scripts/_bump-version-helpers.js` emits `#!/bin/sh` wrapper stubs for `git`/`gh`/`npm`/`pnpm`, uses `which` (Linux/macOS only — Windows is `where`), and joins PATH with `:` instead of `path.delimiter` (`;` on Windows). Adapting the harness for Windows would be a non-trivial rewrite for tests that exercise a release workflow agents shouldn't run on Windows anyway. Added `{ skip: isWindows }` to every test() across 8 bump-version test files; the helper now carries a leading comment explaining the constraint. Eliminates 25+ Windows test failures.
