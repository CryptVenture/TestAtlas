...
- **JSON Schema validator:** **`ajv@^8` + `ajv-formats@^3`** — required because PRD §21 specifies the canonical schema set (V1 baseline of 20, extended in V2 to 39 JSON Schemas in `.testatlas/schemas/*.schema.json` — includes `vocabulary.schema.json` as a first-class member post-Quick-260507-vn2 consolidation); these are real JSON Schema documents, not Zod/Valibot schemas. AJV is the JSON Schema reference implementation in JS. Use Zod/Valibot only for runtime CLI argv validation if at all.
