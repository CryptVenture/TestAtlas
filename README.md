...
- **20 JSON Schemas (Draft 2020-12)** — every machine-readable artifact is schema-governed. `validate-workspace` rejects malformed data before it pollutes your quality intelligence. Note: the vocabulary schema ships at TWO byte-identical paths — `.testatlas/vocabulary.json` (canonical, referenced by command bodies) and `.testatlas/schemas/vocabulary.schema.json` (static-checker / dogfood-audit compatibility copy). Both declare the same `$id` (`https://testatlas.dev/schemas/v1/vocabulary.schema.json`); AJV resolves cross-schema `$ref`s by `$id` URI, not filesystem path. A regression test (`test/schemas/vocabulary-dual-path.test.js`) prevents drift.
...
| 2. First hour | [docs/GETTING_STARTED.md](docs/GETTING_STARTED.md) — walk through all 32 V1 plus 41 V2 `/atlas:*` commands |
