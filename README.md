...
- **20 JSON Schemas (Draft 2020-12)** — every machine-readable artifact is schema-governed. `validate-workspace` rejects malformed data before it pollutes your quality intelligence. Note: `vocabulary.json` (no `.schema.json` filename suffix) IS a canonical schema document — it is registered in AJV by its `$id` (`https://testatlas.dev/schemas/v1/vocabulary.schema.json`), and other schemas reference it via `$ref` to that URI. The filename suffix is convention, not a load-path requirement.
